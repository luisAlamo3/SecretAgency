...
class DiffieHellman:
	...
	def gen_shared_key2(self, other_contribution):
		""" Return g ^ ab mod p """
		# calculate the shared key G^ab mod p
		if self.check_other_public_key(other_contribution):
			self.shared_key = pow(other_contribution, self.__a, self.p)
			return hashlib.sha256(str(self.shared_key).encode()).hexdigest()
			#return hashlib.new('sha256', str(self.shared_key).encode())
		else:
			raise Exception("Bad public key from other party")

alice = DiffieHellman(5)
bob = DiffieHellman(5)
carl = DiffieHellman(5)

alice_pubkey = alice.gen_public_key()
bob_pubkey = bob.gen_public_key()
carl_pubkey = carl.gen_public_key()

alice_sharedkey1 = alice.gen_shared_key(carl_pubkey)
bob_sharedkey1 = bob.gen_shared_key(alice_pubkey)
carl_sharedkey1 = carl.gen_shared_key(bob_pubkey)

alice_sharedkey2 = alice.gen_shared_key2(carl_sharedkey1)
bob_sharedkey2 = bob.gen_shared_key2(alice_sharedkey1)
carl_sharedkey2 = carl.gen_shared_key2(bob_sharedkey1)

print(alice_sharedkey2)
print(bob_sharedkey2)
print(carl_sharedkey2)
print(alice_sharedkey2 == bob_sharedkey2 == carl_sharedkey2)
